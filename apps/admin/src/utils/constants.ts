...
export type OptionDrawStatus =
    (typeof OPTION_DRAW_STATUS)[keyof typeof OPTION_DRAW_STATUS];

export const OPTION_DRAW_STATUS_MAP = Object.freeze({
    [OPTION_DRAW_STATUS.NOT_STARTED]: '未开始',
    [OPTION_DRAW_STATUS.ONGOING]: '进行中',
    [OPTION_DRAW_STATUS.ENDED]: '已结束',
});

export const getOptionDrawStatusLabel = (
    status: OptionDrawStatus | undefined,
) => {
    return status ? OPTION_DRAW_STATUS_MAP[status] : '';
};
