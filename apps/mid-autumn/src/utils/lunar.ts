/**
 * 农历（夏历）换算工具
 *
 * 使用经典的 1900-2099 年农历年历表（lunarInfo），
 * 每个数字编码一个农历年：bit16-4 为 12（闰月为 13）个月的大小月信息，
 * 低 4 位为闰月月份，bit16 为闰月大小。
 *
 * 该表为公开通用算法数据，覆盖 1900-01-31 至 2099 年范围。
 */

const LUNAR_INFO: number[] = [
    0x04bd8,
    0x04ae0,
    0x0a570,
    0x054d5,
    0x0d260,
    0x0d950,
    0x16554,
    0x056a0,
    0x09ad0,
    0x055d2, // 1900-1909
    0x04ae0,
    0x0a5b6,
    0x0a4d0,
    0x0d250,
    0x1d255,
    0x0b540,
    0x0d6a0,
    0x0ada2,
    0x095b0,
    0x14977, // 1910-1919
    0x04970,
    0x0a4b0,
    0x0b4b5,
    0x06a50,
    0x06d40,
    0x1ab54,
    0x02b60,
    0x09570,
    0x052f2,
    0x04970, // 1920-1929
    0x06566,
    0x0d4a0,
    0x0ea50,
    0x06e95,
    0x05ad0,
    0x02b60,
    0x186e3,
    0x092e0,
    0x1c8d7,
    0x0c950, // 1930-1939
    0x0d4a0,
    0x1d8a6,
    0x0b550,
    0x056a0,
    0x1a5b4,
    0x025d0,
    0x092d0,
    0x0d2b2,
    0x0a950,
    0x0b557, // 1940-1949
    0x06ca0,
    0x0b550,
    0x15355,
    0x04da0,
    0x0a5b0,
    0x14573,
    0x052b0,
    0x0a9a8,
    0x0e950,
    0x06aa0, // 1950-1959
    0x0aea6,
    0x0ab50,
    0x04b60,
    0x0aae4,
    0x0a570,
    0x05260,
    0x0f263,
    0x0d950,
    0x05b57,
    0x056a0, // 1960-1969
    0x096d0,
    0x04dd5,
    0x04ad0,
    0x0a4d0,
    0x0d4d4,
    0x0d250,
    0x0d558,
    0x0b540,
    0x0b6a0,
    0x195a6, // 1970-1979
    0x095b0,
    0x049b0,
    0x0a974,
    0x0a4b0,
    0x0b27a,
    0x06a50,
    0x06d40,
    0x0af46,
    0x0ab60,
    0x09570, // 1980-1989
    0x04af5,
    0x04970,
    0x064b0,
    0x074a3,
    0x0ea50,
    0x06b58,
    0x055c0,
    0x0ab60,
    0x096d5,
    0x092e0, // 1990-1999
    0x0c960,
    0x0d954,
    0x0d4a0,
    0x0da50,
    0x07552,
    0x056a0,
    0x0abb7,
    0x025d0,
    0x092d0,
    0x0cab5, // 2000-2009
    0x0a950,
    0x0b4a0,
    0x0baa4,
    0x0ad50,
    0x055d9,
    0x04ba0,
    0x0a5b0,
    0x15176,
    0x052b0,
    0x0a930, // 2010-2019
    0x07954,
    0x06aa0,
    0x0ad50,
    0x05b52,
    0x04b60,
    0x0a6e6,
    0x0a4e0,
    0x0d260,
    0x0ea65,
    0x0d530, // 2020-2029
    0x05aa0,
    0x076a3,
    0x096d0,
    0x04afb,
    0x04ad0,
    0x0a4d0,
    0x1d0b6,
    0x0d250,
    0x0d520,
    0x0dd45, // 2030-2039
    0x0b5a0,
    0x056d0,
    0x055b2,
    0x049b0,
    0x0a577,
    0x0a4b0,
    0x0aa50,
    0x1b255,
    0x06d20,
    0x0ada0, // 2040-2049
    0x14b63,
    0x09370,
    0x049f8,
    0x04970,
    0x064b0,
    0x168a6,
    0x0ea50,
    0x06b20,
    0x1a6c4,
    0x0aae0, // 2050-2059
    0x0a2e0,
    0x0d2e3,
    0x0c960,
    0x0d557,
    0x0d4a0,
    0x0da50,
    0x0d6d6,
    0x0b540,
    0x056d0,
    0x055b2, // 2060-2069
    0x049b0,
    0x0a577,
    0x0a4b0,
    0x0aa50,
    0x1b255,
    0x06d20,
    0x0ada0,
    0x14b63,
    0x09370,
    0x049f8, // 2070-2079
    0x04970,
    0x064b0,
    0x074a3,
    0x0ea50,
    0x06b58,
    0x055c0,
    0x0ab60,
    0x096d5,
    0x092e0,
    0x0c960, // 2080-2089
    0x0d954,
    0x0d4a0,
    0x0da50,
    0x07552,
    0x056a0,
    0x0abb7,
    0x025d0,
    0x092d0,
    0x0cab5,
    0x0a950, // 2090-2099
];

const MS_PER_DAY = 24 * 60 * 60 * 1000;

/** 农历年总天数 */
function lunarYearDays(year: number): number {
    let sum = 348; // 12 * 29
    for (let bit = 0x8000; bit > 0x8; bit >>= 1) {
        sum += LUNAR_INFO[year - 1900] & bit ? 1 : 0;
    }
    return sum + leapDays(year);
}

/** 闰月月份（0 表示无闰月） */
function leapMonth(year: number): number {
    return LUNAR_INFO[year - 1900] & 0xf;
}

/** 闰月天数 */
function leapDays(year: number): number {
    if (!leapMonth(year)) return 0;
    return LUNAR_INFO[year - 1900] & 0x10000 ? 30 : 29;
}

/** 农历某月天数 */
function monthDays(year: number, month: number): number {
    return LUNAR_INFO[year - 1900] & (0x10000 >> month) ? 30 : 29;
}

export interface LunarDate {
    /** 农历年 */
    year: number;
    /** 农历月（1-12） */
    month: number;
    /** 农历日（1-30） */
    day: number;
    /** 是否为闰月 */
    isLeap: boolean;
}

/** 农历年历表支持的公历年份范围（表覆盖 1900-01-31 至 2099-12-31） */
export const LUNAR_MIN_YEAR = 1900;
export const LUNAR_MAX_YEAR = 2099;

/** 超出年历表支持范围时抛出，避免静默返回错误日期 */
function assertLunarSupported(date: Date): void {
    const year = date.getFullYear();
    if (year < LUNAR_MIN_YEAR || year > LUNAR_MAX_YEAR) {
        throw new RangeError(
            `农历换算仅支持 ${LUNAR_MIN_YEAR}–${LUNAR_MAX_YEAR} 年，收到 ${year} 年`,
        );
    }
}

/**
 * 公历转农历（本地时区，按日计算）。
 * 支持范围：1900-01-31 至 2099-12-31。
 */
export function solarToLunar(date: Date): LunarDate {
    assertLunarSupported(date);
    const year = date.getFullYear();
    const base = new Date(1900, 0, 31); // 农历 1900 年正月初一
    let offset = Math.floor(
        (new Date(year, date.getMonth(), date.getDate()).getTime() -
            base.getTime()) /
            MS_PER_DAY,
    );

    let lunarYear = 1900;
    let yearDays = 0;
    for (; lunarYear < 2100 && offset > 0; lunarYear++) {
        yearDays = lunarYearDays(lunarYear);
        offset -= yearDays;
    }
    if (offset < 0) {
        offset += yearDays;
        lunarYear--;
    }

    const leap = leapMonth(lunarYear);
    let isLeap = false;
    let lunarMonth = 1;
    let monthDayCount = 0;

    for (; lunarMonth < 13 && offset > 0; lunarMonth++) {
        // 闰月
        if (leap > 0 && lunarMonth === leap + 1 && !isLeap) {
            lunarMonth--;
            isLeap = true;
            monthDayCount = leapDays(lunarYear);
        } else {
            monthDayCount = monthDays(lunarYear, lunarMonth);
        }

        if (isLeap && lunarMonth === leap + 1) isLeap = false;
        offset -= monthDayCount;
    }

    if (offset === 0 && leap > 0 && lunarMonth === leap + 1) {
        if (isLeap) {
            isLeap = false;
        } else {
            isLeap = true;
            lunarMonth--;
        }
    }

    if (offset < 0) {
        offset += monthDayCount;
        lunarMonth--;
    }

    return {
        year: lunarYear,
        month: lunarMonth,
        day: offset + 1,
        isLeap,
    };
}

const CHINESE_NUMBERS = [
    '正',
    '二',
    '三',
    '四',
    '五',
    '六',
    '七',
    '八',
    '九',
    '十',
    '冬',
    '腊',
];

const CHINESE_DAYS = [
    '初一',
    '初二',
    '初三',
    '初四',
    '初五',
    '初六',
    '初七',
    '初八',
    '初九',
    '初十',
    '十一',
    '十二',
    '十三',
    '十四',
    '十五',
    '十六',
    '十七',
    '十八',
    '十九',
    '二十',
    '廿一',
    '廿二',
    '廿三',
    '廿四',
    '廿五',
    '廿六',
    '廿七',
    '廿八',
    '廿九',
    '三十',
];

const HEAVENLY_STEMS = [
    '甲',
    '乙',
    '丙',
    '丁',
    '戊',
    '己',
    '庚',
    '辛',
    '壬',
    '癸',
];
const EARTHLY_BRANCHES = [
    '子',
    '丑',
    '寅',
    '卯',
    '辰',
    '巳',
    '午',
    '未',
    '申',
    '酉',
    '戌',
    '亥',
];
const ZODIAC = [
    '鼠',
    '牛',
    '虎',
    '兔',
    '龙',
    '蛇',
    '马',
    '羊',
    '猴',
    '鸡',
    '狗',
    '猪',
];

/** 农历月的中文名称，如「八月」「闰四月」 */
export function lunarMonthName(month: number, isLeap: boolean): string {
    return `${isLeap ? '闰' : ''}${CHINESE_NUMBERS[month - 1]}月`;
}

/** 农历日的中文名称，如「十五」「廿三」 */
export function lunarDayName(day: number): string {
    return CHINESE_DAYS[day - 1];
}

/** 干支纪年，如「甲辰」 */
export function ganZhiYear(year: number): string {
    const index = (year - 4) % 60;
    return `${HEAVENLY_STEMS[index % 10]}${EARTHLY_BRANCHES[index % 12]}`;
}

/** 生肖，如「龙」 */
export function zodiacYear(year: number): string {
    return ZODIAC[(year - 4) % 12];
}

/**
 * 找到某一公历年份对应的中秋节（农历八月十五，非闰月）公历日期。
 * 从该年 8 月起逐日扫描（中秋最早出现在 9 月初，最晚 10 月上旬）。
 */
export function findMidAutumn(solarYear: number): Date {
    // 农历八月初一不会早于公历 8 月下旬，从 8 月 1 日开始扫描足够安全
    const cursor = new Date(solarYear, 7, 1);
    const end = new Date(solarYear, 11, 31);

    while (cursor <= end) {
        const lunar = solarToLunar(cursor);
        if (!lunar.isLeap && lunar.month === 8 && lunar.day === 15) {
            return new Date(cursor);
        }
        cursor.setDate(cursor.getDate() + 1);
    }

    throw new Error(`未能找到 ${solarYear} 年的中秋节日期`);
}
