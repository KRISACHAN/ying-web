            ...
            .map((subdir) => {
                try {
                    const packageJson = JSON.parse(
                        fs.readFileSync(path.join(subdir.path, 'package.json'), 'utf8')
                    )
                    return {
...
async function selectBuildTarget() {
    ...
    const modePrompt = new Select({
        ...
        choices: [
            ...
            { name: 'group', message: 'Project group', hint: 'Build all in a directory' },
            { name: 'single', message: 'Single project', hint: 'Build one project' }
        ]
    })
